package com.brain.retenu.web.rest;
...
import com.brain.retenu.config.KafkaProperties;
...
class RetenuKafkaResourceIT {
    ...
    void setup() {
        ...
        RetenuKafkaResource kafkaResource = new RetenuKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/retenu-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/retenu-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
